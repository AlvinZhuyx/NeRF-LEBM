# Multi-gpu with DDP
# using a inference network to do amortized inference
# date 9.3.2021
# this version is to work with entire srn dataset (work with load_srn_all.py)


# add noise term and gaussian regularization term to prior sampling; enabling multiple gpu run 
...
def render(H, W, K, chunk=1024*32, rays=None, z_app=None, z_shape=None, c2w=None, ndc=True,
    ...
    #print(z_app.shape, z_shape.shape)
...
def render_path(render_poses, z_app, z_shape, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, global_step=0, type='sample', rank=0, saveimg=True):
    ...
    for i, c2w in enumerate(render_poses):
        #print('this is ', i, time.time() - t, 'print ends here')
        t = time.time()
...
def create_nerf(args):
    ...
    ebm_app = EBM(args.z_app_dim, 128, nout=1, prior_type=args.ptype_app, act_type=args.ebm_act, normalize=args.normalize_ebm_z).cuda()
    ebm_shape = EBM(args.z_shape_dim, 256, nout=1, prior_type=args.ptype, act_type=args.ebm_act, normalize=args.normalize_ebm_z).cuda()
    
    inference_model = inference(args.z_app_dim, args.z_shape_dim, use_DDP=True).cuda()


    encoder_app = EnCoder(input_ch=args.z_app_dim).cuda()
    encoder_shape = EnCoder(input_ch=args.z_shape_dim).cuda() 
    grad_vars += list(encoder_app.parameters()) + list(encoder_shape.parameters()) + list(inference_model.parameters())
    ...
    return render_kwargs_train, render_kwargs_test, start, grad_vars, grad_vars_ebm, optimizer, optimizer_ebm, ebm_app, ebm_shape, encoder_app, encoder_shape, inference_model
...
def render_rays(ray_batch,
    ...
    if perturb > 0.:
        ...
        #print(z_vals.get_device(), lower.get_device(), upper.get_device(), t_rand.get_device())
...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, type=str, default='./configs/carla_vae_gaussian3_wnoise.txt',
    ...
    parser.add_argument('--ptype_app', default='ebm', type=str, help='prior type for appearance')
    parser.add_argument('--ebm_act', default='relu', type=str, help='activation used in latent ebm')
    parser.add_argument('--normalize_ebm_z', default=False, type=bool)
    ...
    parser.add_argument('--ebm_weight', default=1.0, type=float, help='weight for ebm prior')
    parser.add_argument('--alpha_act', default='relu', type=str, help='activation function for alpha channel')
...
def train(args):
    ...
    # Create nerf model
    render_kwargs_train, render_kwargs_test, start, grad_vars, grad_vars_ebm, optimizer, optimizer_ebm, ebm_app, ebm_shape, encoder_app, encoder_shape, inference_model\
     = create_nerf(args)
    ...
    for i in range(start, args.n_epochs):
        ...
        for imgs, poses, idx in dataloader:
            ...
            imgs = imgs.cuda(non_blocking=True) # imgs should in shape (N, 3, H, W)
            poses = poses.cuda(non_blocking=True) # poses should in shape (N, 4, 4)
            ...
            for ray_rgb in rays_rgb:
                ...
                #batch_rays_rgb_all.append(ray_rgb)
            
            #batch_rays_rgb_all = torch.stack(batch_rays_rgb_all, 0)
            batch_rays_rgb = torch.stack(batch_rays_rgb, 0) #(N, N_rand_per_img, ro+rd+rgb, 3)
            
            #batch_rays_all = batch_rays_rgb_all[:, :, :2, :]
            ...
            for iii in range(args.update_steps):
                z_app, z_shape, KL_loss = inference_model(imgs_clone, use_sample=True)
                z_app = torch.mean(torch.reshape(z_app, (b_sz, args.n_views, args.z_app_dim)), dim=1)
                z_app_normalized = 30 * F.normalize(z_app, dim=-1)

                z_shape = torch.mean(torch.reshape(z_shape, (b_sz, args.n_views, args.z_shape_dim)), dim=1)
                z_shape_normalized = 30 * F.normalize(z_shape, dim=-1)
                
                z_app_encoded = encoder_app(z_app_normalized)
                z_shape_encoded = encoder_shape(z_shape_normalized)
                z_app_encoded = torch.reshape(torch.unsqueeze(z_app_encoded, dim=1).repeat(1, args.n_views, 1), (b_sz * args.n_views, args.z_app_dim))
                z_shape_encoded = torch.reshape(torch.unsqueeze(z_shape_encoded, dim=1).repeat(1, args.n_views, 1), (b_sz * args.n_views, args.z_shape_dim))
                ...
                optimizer.zero_grad()
                img_loss = img2mse(rgb, target_s) #/ (args.g_llhd_sigma ** 2) * args.N_rand / args.batch_size
                trans = extras['raw'][...,-1]
                loss = img_loss * 0.5 / (args.g_llhd_sigma ** 2) * args.N_rand / args.batch_size + KL_loss + args.ebm_weight * (en_pos_shape + en_pos_app)
                psnr = mse2psnr(img_loss)
                #psnr = mse2psnr(img_loss * (args.g_llhd_sigma ** 2) * args.batch_size / args.N_rand)

                if 'rgb0' in extras:
                    img_loss0 = img2mse(extras['rgb0'], target_s) 
                    loss = loss + img_loss0 * 0.5 / (args.g_llhd_sigma ** 2) * args.N_rand / args.batch_size
            ...
            if 'ebm' in args.ptype:
                for _ in range(e_l_steps):
                    en = ebm_app(z_app_neg).sum() + ebm_shape(z_shape_neg).sum() + 0.5 * torch.sum(z_shape_neg ** 2) + 0.5 * torch.sum(z_app_neg ** 2)
                    ...
                    if args.e_l_with_noise:
                        z_app_neg.data = z_app_neg.data + args.e_l_step_size * torch.randn_like(z_app_neg).data
                        z_shape_neg.data = z_shape_neg.data + args.e_l_step_size * torch.randn_like(z_shape_neg).data
                ...
                with torch.no_grad():
                    ...
                    loss_e_app = en_pos_app - en_neg_app
                    loss_e = loss_e_shape + loss_e_app
            # print('time for ebm {}'.format(time.time() - time1))
            ...
            if 'ebm' in args.ptype:
                ...
                new_lrate_ebm = args.lrate_ebm * (0.5 ** (float(global_step) / 30000))
            ...
            if True and (global_step % args.i_test == 0):
                ...
                for iii in range(test_num_batch):
                    ...
                    if 'ebm' in args.ptype:
                        for _ in range(e_l_steps):
                            en = ebm_app(z_app_neg_tmp).sum() + ebm_shape(z_shape_neg_tmp).sum() + 0.5 * torch.sum(z_shape_neg_tmp ** 2) + 0.5 * torch.sum(z_app_neg_tmp ** 2)
                            grad_app, grad_shape = torch.autograd.grad(en, [z_app_neg_tmp, z_shape_neg_tmp])
                            z_app_neg_tmp.data = z_app_neg_tmp.data - 0.5 * args.e_l_step_size ** 2 * grad_app
                            z_shape_neg_tmp.data = z_shape_neg_tmp.data - 0.5 * args.e_l_step_size ** 2 * grad_shape 
                            
                            if args.e_l_with_noise:
                                z_app_neg_tmp.data = z_app_neg_tmp.data + args.e_l_step_size * torch.randn_like(z_app_neg_tmp).data
                                z_shape_neg_tmp.data = z_shape_neg_tmp.data + args.e_l_step_size * torch.randn_like(z_shape_neg_tmp).data
                    ...
                    with torch.no_grad():
                        gen_img, _ = render_path(pose_tmp, z_app_neg_tmp_encoded, z_shape_neg_tmp_encoded, hwf, K, args.chunk, render_kwargs_test, savedir=savedir, global_step=global_step, type='recons', rank = args.local_rank, saveimg=False)
                ...
                if device == torch.device('cuda', 0):
                    #print(gen_imgs.max(), gen_imgs.min(), gen_imgs.shape)
                    if fid < best_fid:
                        best_fid = fid
                        save_best = True
            ...
            if True and global_step % args.i_sample==0: #and device == torch.device('cuda', 0):
                ...
                with torch.no_grad():
                    ...
                    render_path(render_poses, z_app_neg_encoded[:1, :], z_shape_neg_encoded[:1, :], hwf, K, args.chunk, render_kwargs_test, savedir=savedir, global_step=global_step, type='sample', rank = args.local_rank, saveimg=(device == torch.device('cuda', 0)))
                
                t_imgs = []
                with torch.no_grad():
                    for iii in range(5):
                        for jjj in range(5):
                            t_img, _ = render_path(render_poses[1:2, :, :], z_app_neg_encoded[iii:iii+1, :], z_shape_neg_encoded[jjj:jjj+1, :], hwf, K, args.chunk, render_kwargs_test, savedir=savedir, global_step=global_step, type='sample', rank = args.local_rank, saveimg=False)
            ...
            if True and global_step % args.i_testset==0: 
                if device == torch.device('cuda', 0):
                    print('{} begin test'.format(device))
                z_app, z_shape, KL_loss = inference_model(imgs_clone, use_sample=False)
                # Reconstruct image
                with torch.no_grad():
                    z_app = torch.mean(torch.reshape(z_app, (b_sz, args.n_views, args.z_app_dim)), dim=1)
                    z_shape = torch.mean(torch.reshape(z_shape, (b_sz, args.n_views, args.z_shape_dim)), dim=1)
                    ...
                    z_app_encoded_tile = torch.reshape(torch.unsqueeze(z_app_encoded, dim=1).repeat(1, args.n_views, 1), (b_sz * args.n_views, args.z_app_dim))
                    z_shape_encoded_tile = torch.reshape(torch.unsqueeze(z_shape_encoded, dim=1).repeat(1, args.n_views, 1), (b_sz * args.n_views, args.z_shape_dim))
                    render_path(poses, z_app_encoded_tile, z_shape_encoded_tile, hwf, K, args.chunk, render_kwargs_test, savedir=savedir, global_step=global_step, type='recons', rank = args.local_rank, saveimg=(device == torch.device('cuda', 0)))
                    render_path(render_poses, z_app_encoded_tile[:1, :], z_shape_encoded_tile[:1, :], hwf, K, args.chunk, render_kwargs_test, savedir=savedir, global_step=global_step, type='novel_view', rank = args.local_rank, saveimg=(device == torch.device('cuda', 0)))
                ...
                if device == torch.device('cuda', 0):
                    torchvision.utils.save_image(imgs_grid.clone(), os.path.join(savedir, '{}_{:06d}_{}_ori_train.png'.format('recons', global_step, args.local_rank)))
                

            if  global_step % args.i_print==0 and device == torch.device('cuda', 0):
                dt = time.time()-time0
                print(f"[TRAIN] Iter: {i} step {global_step} Time: {dt:.2f} Loss: {loss.item():.3f}  PSNR: {psnr.item():.2f} {psnr0.item():.2f}  KL: {KL_loss.item():.2f} en_sp: {en_pos_shape.item():.3f} en_sn: {en_neg_shape.item():.3f} en_ap: {en_pos_app.item():.3f} en_an: {en_neg_app.item():.3f}")
                time0 = time.time()
