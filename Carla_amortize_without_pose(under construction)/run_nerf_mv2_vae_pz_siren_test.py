...
# try to treat the pose also as a variable to infer
...
from run_nerf_helpers_pz_siren import *
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    if activation == 'sigmoid':
        ...
        #print(raw[...,:3].max(), raw[...,:3].min(), torch.abs(raw[...,:3]).mean(), rgb.max(), rgb.min(), rgb.mean())
        #assert False
    else:
...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, type=str, default='./configs/config_test_carla.txt',
    ...
    parser.add_argument('--mode', type=str, default='train', help='train or generate or infer pose')
...
def train(args):
    ...
    if not args.train:
        ...
        data_size = 10000
        print('Synthesizing mode')
        syndir = os.path.join(basedir, expname, 'synthesis')
        ...
        print('Calculating pose statistics')
        for imgs, poses, idx in dataloader:
            imgs = imgs.cuda(non_blocking=True)
            imgs = torch.reshape(imgs, (-1, 3, H, W))
            poses_infer, _, theta, phi, _ = pose_estimator(imgs, use_sample=False, return_angle=True)

            poses_array = poses.detach().cpu().numpy()
            assert poses_array.shape == (len(poses_array), 4, 4)

            last_vec = poses_array[:, :, -1] / radius
            phi_true = np.arcsin(-last_vec[:, 2])
            cos_phi_true = np.cos(phi_true)
            sin_theta_true = last_vec[:, 0] / (cos_phi_true + 1e-8)
            cos_theta_true = last_vec[:, 1] / (cos_phi_true + 1e-8)
            theta_true = np.arcsin(sin_theta_true, cos_theta_true)
            
            theta_list.append(theta)
            phi_list.append(phi)
            theta_true_list.append(theta_true)
            phi_true_list.append(phi_true)
            img_list.append(imgs.clone().detach()) 
            data_counter += len(imgs)
            print(data_counter)
            if data_counter >= data_size:
                break
        img_list = torch.cat(img_list, 0)
        if device == torch.device('cuda', 0):
            imgs_grid = torchvision.utils.make_grid(img_list[:100], nrow=10)
            torchvision.utils.save_image(imgs_grid.clone(), os.path.join(syndir, 'obs_img.png'))
        
        theta_list = np.concatenate(theta_list) 
        phi_list = np.concatenate(phi_list)
        theta_true_list = np.concatenate(theta_true_list) 
        phi_true_list = np.concatenate(phi_true_list)
        phi_list[phi_list < 0] += 2 * np.pi # align to [0, 2*np.pi]
        phi_true_list[phi_true_list < 0] += 2 * np.pi
        plt.figure()
        plt.hist(theta_list / np.pi * 180, bins=60)
        plt.savefig(os.path.join(syndir, 'theta_hist.png'))
        plt.close()
        plt.figure()
        plt.hist(phi_list / np.pi * 180, bins=60)
        plt.savefig(os.path.join(syndir, 'phi_hist.png'))
        plt.close()
        # generate image
        start_time = time.time()
        print('Begin random generate image')
        # we sample the theta and phi according to their observed frequency in the training data
        p_theta, range_theta = np.histogram(theta_list, bins=200, density=True)
        p_phi, range_phi = np.histogram(phi_list, bins=200, density=True)
        p_theta = p_theta / np.sum(p_theta)
        p_phi = p_phi / np.sum(p_phi)
        v_theta = 0.5 * (range_theta[1:] + range_theta[:-1])
        v_phi = 0.5 * (range_phi[1:] + range_phi[:-1])
        syn_imgs = []

        # randomly sample images
        for i in range(10):
            batch_size = 100
            z_neg_shape = torch.autograd.Variable(args.sigma * sample_p0(batch_size, args.z_shape_dim), requires_grad=True) # using more samples in estimate the expectation
            z_neg_app = torch.autograd.Variable(args.sigma * sample_p0(batch_size, args.z_app_dim), requires_grad=True)
            ...
            for _ in range(batch_size):
                tmp_theta = np.random.choice(v_theta, p=p_theta)
                tmp_phi = np.random.choice(v_phi, p=p_phi)
                poses.append(dataset.angle2pose(tmp_theta, tmp_phi))
            poses = torch.stack(poses, 0).cuda(non_blocking=True)
            
            with torch.no_grad():
                rendered_imgs, _ = render_path(poses, z_neg_app, z_neg_shape, hwf, K, args.chunk, render_kwargs_test, savedir=savedir, global_step=global_step, type='recons', rank = args.local_rank, saveimg=False)
            syn_imgs.append(rendered_imgs.clone().detach().cpu())

            if i < 10 and device == torch.device('cuda', 0):
                imgs_grid = torchvision.utils.make_grid(rendered_imgs, nrow=int(np.sqrt(batch_size)))
                torchvision.utils.save_image(imgs_grid.clone(), os.path.join(syndir, 'samples{}.png'.format(i)))
            if device == torch.device('cuda', 0):
                print('Finish {} time {:.3f}'.format(i, time.time() - start_time))

        # sample images and rotate camera pose
        z_neg_shape = torch.autograd.Variable(args.sigma * sample_p0(30, args.z_shape_dim), requires_grad=True) # using more samples in estimate the expectation
        z_neg_app = torch.autograd.Variable(args.sigma * sample_p0(30, args.z_app_dim), requires_grad=True)
        e_l_steps = args.e_l_steps
        if 'ebm' in args.ptype:
            for _ in range(e_l_steps):
                en = ebm_shape(z_neg_shape).sum() + 0.5 * torch.sum(z_neg_shape ** 2) + ebm_app(z_neg_app).sum() + 0.5 * torch.sum(z_neg_app ** 2)
        ...
        z_neg_app = z_neg_app.detach()
        z_neg_shape = z_neg_shape.detach()
        render_poses = dataset.get_render_poses(max_theta= 0, min_theta=-np.pi, max_phi=np.pi * 0.9, min_phi=0.6 * np.pi, ntheta=12, nphi=12)
        # generate image 
        for i in range(len(z_neg_app)):
            with torch.no_grad():
                t_imgs, _ = render_path(render_poses, z_neg_app[i: (i+1), :], z_neg_shape[i: (i+1), :], hwf, K, args.chunk, render_kwargs_test, savedir=syndir, global_step=global_step, type='sample', rank = args.local_rank, saveimg=False)
                t_imgs = torchvision.utils.make_grid(t_imgs, nrow=12)
                if device == torch.device('cuda', 0):
                    torchvision.utils.save_image(t_imgs.clone(), os.path.join(syndir, 'sample_fine{}.png'.format(i))) 
        return
    ...
    for i in range(start + 1, args.n_epochs):
        ...
        for imgs, poses, idx in dataloader:
            ...
            if True and global_step % args.i_sample==0: #and device == torch.device('cuda', 0):
                ...
                render_poses = dataset.get_render_poses(max_theta=theta_pred[0] + 90./180 * np.pi, min_theta=theta_pred[0] - 90./180 * np.pi, max_phi=phi_pred[0] + 40./180 * np.pi, min_phi=phi_pred[0] - 40./180 * np.pi, ntheta=11, nphi=11)
                with torch.no_grad():
            ...
            if True and global_step % args.i_testset==0: #and (device == torch.device('cuda', 0)):
                # in the case that we need to infer pose, the inferred pose may not be aligned with the given pose
                # thus, instead of doing validation on given poses, we actually need to do all the poses (i.e. rendered_poses) 
                if device == torch.device('cuda', 0):
                ...
                for iii in range(min(2, len(z_app))):
                    gen_imgs = []
                    gen_disps = []
                    render_poses = dataset.get_render_poses(max_theta=theta_pred[iii] + 90./180 * np.pi, min_theta=theta_pred[iii] - 90./180 * np.pi, max_phi=phi_pred[iii] + 40./180 * np.pi, min_phi=phi_pred[iii] - 40./180 * np.pi, ntheta=11, nphi=11)
                    with torch.no_grad():
