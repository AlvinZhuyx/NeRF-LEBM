...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, type=str, default='./configs/config_train_carla.txt',
...
def train(args):
    ...
    if args.dataset_type == 'carla':
    ...
    elif args.dataset_type == 'srn':
        from load_srn_ori import ImageDataset, TestDataset
        near = 0.8
        far = 1.8
        args.radius = 1.3
        dataset = ImageDataset(data_dirs=args.datadir,  im_sz=args.im_size, n_views=args.n_views)
        if device == torch.device('cuda', 0):
            testset = TestDataset(data_dirs=args.datadir, im_sz=args.im_size)
        else:
            testset = TestDataset(data_dirs=args.datadir, im_sz=args.im_size)
    ...
    elif args.dataset_type == 'srn_chair':
        from load_srn_chair import ImageDataset, TestDataset
        near = 1.25
        far = 2.75
        args.radius = 2.0
        dataset = ImageDataset(data_dirs=args.datadir,  im_sz=args.im_size, n_views=args.n_views)
        ...
        from load_celeba import ImageDataset
        near = 7.5
        far = 12.5
        args.radius = 10.0
        dataset = ImageDataset(data_dirs=args.datadir,  im_sz=args.im_size, n_views=args.n_views)
        testset = None
        args.data_number = len(dataset)
        print('Load celeba data total number of training data ', args.data_number)
        train_sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=True)
        dataloader = torch.utils.data.DataLoader(
                        dataset,
                        batch_size=args.batch_size,
    ...
    # Create nerf model
    '''
    render_kwargs_train, render_kwargs_test, start, grad_vars, grad_vars_ebm, optimizer, optimizer_ebm, ebm_app, ebm_shape, inference_model, pose_estimator\
    '''
    ...
    # render_poses = dataset.get_render_poses(2 * np.pi, 0.0, 2 * np.pi, 0.0, 20, 20)
    ...
    for i in range(start + 1, args.n_epochs):
        ...
        for imgs, poses, idx in dataloader:
            ...
            for iii in range(args.update_steps):
                ...
                img_loss = img2mse(rgb, target_s) #/ (g_llhd_sigma ** 2) * args.N_rand / args.batch_size
                trans = extras['raw'][...,-1]
                
                loss = img_loss * 0.5 / (g_llhd_sigma ** 2) * args.N_rand / args.batch_size + KL_loss + KL_pose + en_pos_shape + en_pos_app
                psnr = mse2psnr(img_loss)
                #psnr = mse2psnr(img_loss * (g_llhd_sigma ** 2) * args.batch_size / args.N_rand)
